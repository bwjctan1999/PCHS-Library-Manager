...
Public Class UC_Reports
    ...
    Private Sub Load_Borrowed_Data()
        ...
        Try
            ...
            booktotal = cmd.ExecuteScalar

            command("SELECT COUNT(*) FROM tbl_borrowed_books")
            borrowedtotal = cmd.ExecuteScalar
    ...
    Private Sub Load_StickyNotes()
        Try
            ...
            While reader.Read()
                UC_SNHolder1.AddNote(reader("text"), reader("location"), reader("size"))
            End While
    ...
    Public Sub SaveNotes()
        Try
            ...
            For Each note In UC_SNHolder1.GetNotes()
                ...
                command("INSERT INTO tbl_stickynotes (`text`, `location`, `size`, `librarianID`) VALUES (@text, @loc, @size, @libid)")
                cmd.Parameters.AddWithValue("@text", note.GetRTF())
